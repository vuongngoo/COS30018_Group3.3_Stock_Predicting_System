...
DATA_PATH = os.path.join("data_cache", "preprocessed_tensors_sentiment.npz")
BASE_EXP_DIR = "experiments_sentiment"
...
class SentimentDirectionClassifier(nn.Module):
    def __init__(self, model_type: str, input_size: int, hidden_size: int, num_layers: int, dropout: float):
        super(SentimentDirectionClassifier, self).__init__()
...
CONFIGURATIONS = [
    {
        ...
        "dropout": 0.2,
        "patience": 10,
        "desc": "Single layer with BCE loss on technical + sentiment features"
    },
    {
        "name": "config_2_deep",
        "lr": 0.0003,
        ...
        "dropout": 0.3,
        "patience": 12,
        "desc": "2-layer stacked RNN with dropout 0.3 on sentiment features"
    }
]
...
def load_data_loaders(data_path: str, batch_size: int):
    if not os.path.exists(data_path):
        raise FileNotFoundError(f"'{data_path}' not found! Ensure data_preprocess_sentiment.py has been executed.")
...
def train_single_model(model_type: str, cfg: dict, save_dir: str, train_loader, val_loader, test_loader, num_features: int):
    ...
    model = SentimentDirectionClassifier(
    ...
    print(f"\n  [Training {model_type.upper()} Classifier ({num_features} Sentiment features)]...")
    ...
    plt.title(f'Sentiment Loss Curve: {model_type.upper()} ({cfg["name"]})')
    plt.xlabel('Epochs')
    plt.ylabel('BCE Loss')
    plt.legend()
...
def main():
    ...
    for cfg in CONFIGURATIONS:
        ...
        print(f" SENTIMENT CONFIGURATION: {config_folder_name.upper()}")
    ...
    csv_path = os.path.join(BASE_EXP_DIR, "sentiment_metrics_summary.csv")
    metrics_df.to_csv(csv_path, index=False)
    print(f"\n Saved classification metrics summary CSV to: '{csv_path}'")



    print(" SENTIMENT CLASSIFICATION EXPERIMENT SUMMARY")
